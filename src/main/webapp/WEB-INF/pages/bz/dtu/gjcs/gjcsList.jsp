...
告警类型：<input type="text" name="gjlx" id="gjlx" style="height:25px;width: 400px; border:1px solid #CCC;"/>
<!-- 改 -->
...
<table id="dg" class="easyui-datagrid" title="告警功能调试" style="width:100%;height:auto"
...
<script type="text/javascript">
    ...
    $(function () {
        $('#dg').datagrid({
            ...
            url: '${basePath}/bzgjcs/selectByPage',             // 改
            method: 'get',
            iconCls: 'icon-ok',
            nowrap: false,
            ...
            columns: [[
                ...
                {field: 'zzlxname', halign: 'center', align: "center", title: '装置类型', width: '40%'},
                {field: 'jg', halign: 'center', align: "center", title: '间隔', width: '10%'},        // 改
                {field: 'gjlx', halign: 'center', align: "center", title: '告警类型', width: '25%'},     // 改
                {
                    field: 'trans',
                    halign: 'center',
                    align: "left",
                    ...
                    formatter: function (value, row, index) {
                        let txt1 = '&nbsp;&nbsp;<button href="javascript:void(0);" onclick="updRecord(\'' + row.id + '\')" class="iput_m" style="width: 40px; height: 20px;">' + '修改' + '</button>';
                        let txt3 = '&nbsp;&nbsp;<button href="javascript:void(0);" onclick="delRecord(\'' + row.id + '\')" class="iput_m" style="width: 40px; height: 20px;">' + '删除' + '</button>';
                        if(bzglFun != 2) return '';
                        return txt1 + txt3;
                    }
                },
...
<script type="text/javascript">
    ...
    function addRecord() {
        ...
        window.open("${basePath}/bz/dtu/gjcs/addGjcs", "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function updRecord(id) {
        var w = window.screen.width / 2 - 950 / 2;
        var t = window.screen.height / 2 - 600 / 2 - 35;
        window.open("${basePath}/bz/dtu/gjcs/updGjcs/" + id, "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function delRecord(id) {
        ...
        $.ajax({
            url: '${basePath}/bzgjcs/deleteByPrimaryKey/' + id,      // 改
            type: 'POST',
            async: false,
    ...
    function selRecord() {
        ...
        queryParams.gjlx = $("#gjlx").val();        // 改

        $('#dg').datagrid('options').url = '${basePath}/bzgjcs/selectByPage';       // 改
