...
被测回路：<input type="text" name="bchl" id="bchl" style="height:25px;width: 300px; border:1px solid #CCC;"/>
<!-- 改 -->
试验电压（伏）：<input type="text" name="sydy" id="sydy" style="height:25px;width: 400px; border:1px solid #CCC;"/>
<!-- 改 -->
...
<table id="dg" class="easyui-datagrid" title="绝缘电阻测试标准" style="width:100%;height:auto"
       data-options=""></table>
...
<script type="text/javascript">
    ...
    $(function () {
        $('#dg').datagrid({
            ...
            url: '${basePath}/bzjydz/selectByPage',             // 改
            method: 'get',
            iconCls: 'icon-ok',
            nowrap: false,
            ...
            columns: [[
                ...
                {field: 'bchl', halign: 'center', align: "center", title: '被测回路', width: '50%'},        // 改
                {field: 'sydy', halign: 'center', align: "center", title: '试验电压（伏）', width: '25%'},     // 改
                {
                    field: 'trans',
                    halign: 'center',
                    align: "left",
...
<script type="text/javascript">
    ...
    function addRecord() {
        ...
        window.open("${basePath}/bz/dtu/jydz/addJydz", "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function updRecord(id) {
        var w = window.screen.width / 2 - 950 / 2;
        var t = window.screen.height / 2 - 600 / 2 - 35;
        window.open("${basePath}/bz/dtu/jydz/updJydz/" + id, "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function delRecord(id) {
        ...
        $.ajax({
            url: '${basePath}/bzjydz/deleteByPrimaryKey/' + id,      // 改
            type: 'POST',
            async: false,
    ...
    function selRecord() {
        ...
        queryParams.bchl = $("#bchl").val();        // 改
        queryParams.sydy = $("#sydy").val();        // 改

        $('#dg').datagrid('options').url = '${basePath}/bzjydz/selectByPage';       // 改
