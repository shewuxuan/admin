...
<body>
<h2>模拟CT一次电流验证回路</h2>                                           <!-- 改 -->
所属区域：<select name="ssqy" id="ssqy" style="height:25px; border:1px solid #CCC;"></select>
间隔：<input type="text" name="jg" id="jg" style="height:25px;width: 100px; border:1px solid #CCC;"/>        <!-- 改 -->
测试功能：<input type="text" name="csgn" id="csgn" style="height:25px;width: 400px; border:1px solid #CCC;"/>
<!-- 改 -->
测量方法：<input type="text" name="csff" id="csff" style="height:25px;width: 400px; border:1px solid #CCC;"/>
<!-- 改 -->
...
<table id="dg" class="easyui-datagrid" title="模拟CT一次电流验证回路" style="width:100%;height:auto"
...
<script type="text/javascript">
    ...
    $(function () {
        $('#dg').datagrid({
            ...
            url: '${basePath}/bzdlhl/selectByPage',             // 改
            method: 'get',
            iconCls: 'icon-ok',
            nowrap: false,
            ...
            columns: [[
                ...
                {field: 'zzlxname', halign: 'center', align: "center", title: '装置类型', width: '25%'},
                {field: 'jg', halign: 'center', align: "center", title: '间隔', width: '10%'},        // 改
                {field: 'csgn', halign: 'center', align: "center", title: '测试功能', width: '15%'},        // 改
                {field: 'csff', halign: 'center', align: "center", title: '测量方法', width: '25%'},     // 改
                {
                    field: 'trans',
                    halign: 'center',
                    align: "left",
...
<script type="text/javascript">
    ...
    function addRecord() {
        ...
        window.open("${basePath}/bz/dtu/dlhl/addDlhl", "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function updRecord(id) {
        var w = window.screen.width / 2 - 950 / 2;
        var t = window.screen.height / 2 - 600 / 2 - 35;
        window.open("${basePath}/bz/dtu/dlhl/updDlhl/" + id, "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function delRecord(id) {
        ...
        $.ajax({
            url: '${basePath}/bzdlhl/deleteByPrimaryKey/' + id,      // 改
            type: 'POST',
            async: false,
    ...
    function selRecord() {
        ...
        queryParams.csgn = $("#csgn").val();        // 改
        queryParams.csff = $("#csff").val();        // 改

        $('#dg').datagrid('options').url = '${basePath}/bzdlhl/selectByPage';       // 改
