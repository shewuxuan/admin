    ...
    <div class="right_table" style="width:96%;">
        <form name="myForm" id="myForm">
            <table width="100%" cellpadding="0" cellspacing="1" bgcolor="#c6c6c6">
                ...
                <tr>
                    <th bgcolor="#FFFFFF"><p>试验仪器名称</p></th>        <!-- 改-->
                    <td bgcolor="#FFFFFF" style="text-align:left; ">
                        <input type="text" name="yqmc" id="yqmc" style="height:25px; width: 99%; border:1px solid #CCC;"/>
                        <!-- 改-->
                    </td>
                </tr>
...
<script type="text/javascript">
    $(function () {
        ...
        $.get("${basePath}/pwbh_bz_yqyb/selectByPrimaryKey/" + id, function (data) {		// 改
...
<script type="text/javascript">
    ...
    function commit() {
        let ssqy = $("#ssqy").val();
        let yqmc = $("#yqmc").val();
        if (yqmc === "" || yqmc == null || yqmc.replace(/(^\s*)|(\s*$)/g, "") === "") {
            alert("试验仪器名称不能为空！");
            $("#yqmc").focus();
            return false;
        } else {
            let data = {"id": "${requestScope.id}", "ssqy": ssqy, "yqmc": yqmc}
            $.ajax({
                type: 'POST',
                ...
                dataType: "json",//必须
                url: '${basePath}/pwbh_bz_yqyb/updateByPrimaryKey',		// 改
