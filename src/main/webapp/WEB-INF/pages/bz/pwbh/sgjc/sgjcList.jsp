...
内容：<input type="text" name="nr" id="nr" style="height:25px;width: 300px; border:1px solid #CCC;"/>        <!-- 改 -->
...
<table id="dg" class="easyui-datagrid" title="收工前检查标准" style="width:100%;height:auto"></table>     <!-- 改 -->
...
<script type="text/javascript">
    $(function () {
        $('#dg').datagrid({
            ...
            url: '${basePath}/pwbh_bz_sgjc/selectByPage',             // 改
            method: 'get',
            iconCls: 'icon-ok',
            nowrap: false,
            ...
            columns: [[
                ...
                {field: 'nr', halign: 'center', align: "center", title: '内容', width: '73%'},        // 改
                {
                    field: 'trans',
                    halign: 'center',
                    align: "left",
...
<script type="text/javascript">
    ...
    function addRecord() {
        ...
        window.open("${basePath}/bz/pwbh/sgjc/addSgjc", "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function updRecord(id) {
        var w = window.screen.width / 2 - 950 / 2;
        var t = window.screen.height / 2 - 600 / 2 - 35;
        window.open("${basePath}/bz/pwbh/sgjc/updSgjc/" + id, "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function delRecord(id) {
        ...
        $.get('${basePath}/pwbh_bz_sgjc/deleteByPrimaryKey/' + id, {"id": id}, function (data) {
            data = JSON.parse(data);
            if (data.code === 0) {
                alert("删除成功！");
                $('#dg').datagrid("load");
    ...
    function selRecord() {
        ...
        queryParams.nr = $("#nr").val();        // 改

        $('#dg').datagrid('options').url = '${basePath}/pwbh_bz_sgjc/selectByPage';       // 改
