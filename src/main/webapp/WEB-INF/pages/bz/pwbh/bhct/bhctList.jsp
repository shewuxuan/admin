...
相别：<input type="text" name="xb" id="xb" style="height:25px;width: 300px; border:1px solid #CCC;"/>        <!-- 改 -->
...
<table id="dg" class="easyui-datagrid" title="保护CT标准" style="width:100%;height:auto"></table>     <!-- 改 -->
...
<script type="text/javascript">
    ...
    $(function () {
        $('#dg').datagrid({
            ...
            url: '${basePath}/pwbh_bz_bhct/selectByPage',             // 改
            method: 'get',
            iconCls: 'icon-ok',
            nowrap: false,
            ...
            columns: [[
                ...
                {field: 'xb', halign: 'center', align: "center", title: '相别', width: '43%'},        // 改
                {
                    field: 'trans',
                    halign: 'center',
                    align: "left",
...
<script type="text/javascript">
    ...
    function addRecord() {
        ...
        window.open("${basePath}/bz/pwbh/bhct/addBhct", "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function updRecord(id) {
        var w = window.screen.width / 2 - 950 / 2;
        var t = window.screen.height / 2 - 600 / 2 - 35;
        window.open("${basePath}/bz/pwbh/bhct/updBhct/" + id, "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function delRecord(id) {
        ...
        $.get('${basePath}/pwbh_bz_bhct/deleteByPrimaryKey/' + id, {"id": id}, function (data) {
            data = JSON.parse(data);
            if (data.code === 0) {
                alert("删除成功！");
                $('#dg').datagrid("load");
    ...
    function selRecord() {
        ...
        queryParams.xb = $("#xb").val();        // 改

        $('#dg').datagrid('options').url = '${basePath}/pwbh_bz_bhct/selectByPage';       // 改
