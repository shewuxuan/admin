    ...
    <div class="right_table" style="width:96%;">
        <form name="myForm" id="myForm">
            <table width="100%" cellpadding="0" cellspacing="1" bgcolor="#c6c6c6">
                ...
                <tr>
                    <th bgcolor="#FFFFFF"><p>测试结果</p></th>        <!-- 改-->
                    <td bgcolor="#FFFFFF" style="text-align:left; ">
                        <input type="text" name="csjg" id="csjg" style="height:25px; width: 99%; border:1px solid #CCC;"/>
                        <!-- 改-->
                    </td>
                </tr>

                <tr>
                    <th bgcolor="#FFFFFF"><p>通入要求值</p></th>        <!-- 改-->
                    <td bgcolor="#FFFFFF" style="text-align:left; ">
                        <input type="text" name="" id="tryq" style="height:25px; width: 99%; border:1px solid #CCC;"/>
                        <!-- 改-->
                    </td>
                </tr>
...
<script type="text/javascript">
    $(function () {
        ...
        $.get("${basePath}/pwbh_bz_bhct/selectByPrimaryKey/" + id, function (data) {		// 改
...
<script type="text/javascript">
    ...
    function commit() {
        ...
        let csjg = $("#csjg").val();
        let tryq = $("#tryq").val();
        if (csjg === "" || csjg == null || csjg.replace(/(^\s*)|(\s*$)/g, "") === "") {
            alert("测试结果不能为空！");
            $("#csjg").focus();
            return false;
        } else if (tryq === "" || tryq == null || tryq.replace(/(^\s*)|(\s*$)/g, "") === "") {
            alert("通入要求值不能为空！");
            $("#tryq").focus();
            return false;
        } else {
            let data = {"id": "${requestScope.id}", "ssqy": ssqy, "csjg": csjg, "tryq": tryq}
            $.ajax({
                type: 'POST',
                ...
                dataType: "json",//必须
                url: '${basePath}/pwbh_bz_jdjy/updateByPrimaryKey',		// 改
