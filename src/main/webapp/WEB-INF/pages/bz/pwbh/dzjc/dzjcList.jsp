...
间隔：<input type="text" name="" id="jg" style="height:25px;width: 300px; border:1px solid #CCC;"/>        <!-- 改 -->
...
<table id="dg" class="easyui-datagrid" title="定值检查标准" style="width:100%;height:auto"></table>     <!-- 改 -->
...
<script type="text/javascript">
    $(function () {
        $('#dg').datagrid({
            ...
            url: '${basePath}/pwbh_bz_dzjc/selectByPage',             // 改
            method: 'get',
            iconCls: 'icon-ok',
            nowrap: false,
...
<script type="text/javascript">
    ...
    function addRecord() {
        ...
        window.open("${basePath}/bz/pwbh/dzjc/addDzjc", "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function updRecord(id) {
        var w = window.screen.width / 2 - 950 / 2;
        var t = window.screen.height / 2 - 600 / 2 - 35;
        window.open("${basePath}/bz/pwbh/dzjc/updDzjc/" + id, "_blank", " height=500,width=1000,top=" + t + ",left=" + w + ",toolbar=no,menubar=no,localtion=no,scrollbars=no");     // 改
    }
    ...
    function delRecord(id) {
        ...
        $.get('${basePath}/pwbh_bz_dzjc/deleteByPrimaryKey/' + id, {"id": id}, function (data) {
            data = JSON.parse(data);
            if (data.code === 0) {
                alert("删除成功！");
                $('#dg').datagrid("load");
    ...
    function selRecord() {
        ...
        $('#dg').datagrid('options').url = '${basePath}/pwbh_bz_dzjc/selectByPage';       // 改
