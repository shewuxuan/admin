...
public class StringUtils {
    ...
    /**
     * 生成随机字符串（年月日+时分秒+四位随机数）
     *
     * @return
     */
    public static String createDateRandomString(Date date) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddhhmmss");
        String str = sdf.format(date);
        String t = String.format("%04d", new Random().nextInt(9999));
        return str + t;
    }
     ...
     * @return
     */
    public static String checkParamLength(String field, Integer length, String fieldName, boolean isNull) {
        String falg = "";
        if (!isNull) {
        ...
        if (field.length() > length) {
            falg = fieldName + "最大长度为" + length + "个字符！";
        } else {
            falg = "0";
        }
        return falg;
