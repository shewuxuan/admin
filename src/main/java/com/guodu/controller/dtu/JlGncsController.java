 ...
 * @Description: TODO
 * @Author: 2Uli
 * @Date: 2020/2/20 1:30 下午
 */
@RestController
@RequestMapping("jlgncs/")
public class JlGncsController {
    ...
    public String insert(@RequestBody JlGncs record, HttpServletRequest request) {
        map.clear();
        try {
            ...
            map.put("msg", "添加成功！");
            map.put("data", i);
            StaticLog.info("jl_gncs表【插入】成功 -----> " + record.getId(), "INFO");
        } catch (Exception e) {
            map.put("code", -1);
            map.put("msg", "添加失败！");
            map.put("data", 0);
            StaticLog.error(e, "jl_gncs表【插入】失败 -----> " + record.getId(), "ERROR");
        }
        return JSONUtil.toJsonStr(map);
    }

    @RequestMapping("selectByAll")
    public String selectByAll(JlGncs jlGncs) {
        map.clear();
        try {
            List<JlGncs> jlGncss = jlGncsServiceImpl.selectByAll(jlGncs);
            map.put("code", "0");
            map.put("msg", "查询完成");
    ...
    public String updateBatch(@RequestBody List<JlGncs> list, HttpServletRequest request) {
        map.clear();
        try {
            String czr = Auth.getAuth(request).getLoginname();
            for (JlGncs jlGncs : list) {
                jlGncs.setCzr(czr);
    ...
    public String deleteByPrimaryKey(@PathVariable String id) {
        ...
        try {
            ...
            map.put("code", "0");
            map.put("msg", "删除成功");
            StaticLog.info("jl_gncs表【删除】成功 -----> " + id, "INFO");
        } catch (Exception e) {
            StaticLog.error(e, "jl_gncs表【删除】失败 -----> " + id, "ERROR");
            map.put("code", "-1");
            map.put("msg", "删除失败！");
        }
        return JSONUtil.toJsonStr(map);
