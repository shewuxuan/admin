...
function initDept(verId) {
    $.getJSON("Cxpgbz/DTree1.action", {"cmd": "getAll", verId: verId}, function (data) {
        console.log(data);
        var options = {
            data: data.body,
            ...
            click: function (el, id, type) {
                ...
                console.log(id);
                console.log(type);
                $(window.parent.document).find("#right").attr("src", path + "/TPgfa/toList.action?deptId=" + id + "&type=" + type);
            }
        };
        $("#tree1").DTree(options);
    });
